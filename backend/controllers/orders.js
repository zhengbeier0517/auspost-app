...
export async function listOrders(req, res) {
  ...
  res.json({ orders: store.orders.map(order => buildOrder(order, store.products)), originPostcode: store.originPostcode });
}
export async function getOrder(req, res) {
  const store = await readStore();
  const order = store.orders.find(o => o.orderNo === req.params.orderNo);
  if (!order) return res.status(404).json({ message: 'Order not found.' });
  res.json(buildOrder(order, store.products));
}
export async function getTracking(req, res) {
  ...
  res.json({ shipments: await Promise.all(unique.map(track)) });
}

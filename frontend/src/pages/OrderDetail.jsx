...
export default function OrderDetail({ order, onBack }) {
  ...
  const simulated = order.trackingMode !== 'testbed'
  ...
  return <>
    ...
    <div className="test-banner"><span>TEST ORDER</span>Fictional order and customer · Prices from the supplied product catalog · {simulated ? 'Simulated tracking · No courier API calls' : 'Tracking uses the courier testbed'}</div>
    <div className="detail-grid"><div className="detail-primary">
      ...
      <section className="panel tracking-panel"><div className="section-heading"><div className="section-title"><Icon name="truck"/><h2>Shipment tracking</h2><span className="count-badge">{order.shipments.length}</span></div><button className="text-button" onClick={refresh} disabled={loading}><Icon name="refresh" size={15} className={loading ? 'spin' : ''}/>{loading ? 'Checking…' : 'Refresh'}</button></div>
        ...
        <div aria-live="polite">{order.shipments.map(s => {
          ...
          return <div className="shipment" key={s.trackingId}><div className="shipment-header"><span className={`carrier-icon ${s.carrier === 'TNT' ? 'tnt' : ''}`}>{s.carrier === 'TNT' ? 'TNT' : <Icon name="truck" size={22}/>}</span><div><h3>{s.carrier === 'StarTrack' ? 'StarTrack / Australia Post' : s.carrier}</h3><div className="shipment-number">{s.trackingNumber}<span>·</span>{s.trackingId}</div></div><span className="subtle-chip">{simulated ? 'Simulated' : 'Testbed'}</span></div>
            {loading ? <div className="tracking-state" role="status"><Icon name="clock"/><div><strong>{simulated ? 'Loading simulated tracking' : 'Checking the courier testbed'}</strong><p>Retrieving the latest available response…</p></div></div> : result?.availability === 'available' ? <><div className="live-status"><span className="green-dot"/>{result.status}</div><div className="timeline">{result.events.map((e,i) => <div className="timeline-item" key={i}><span className="timeline-dot"/><strong>{e.description}</strong><p>{e.location || 'Location not provided'}</p><small>{eventDate(e.date)}</small></div>)}</div><p className="tracking-caption">Last event: {eventDate(result.lastUpdated)} · {simulated ? 'Simulated events, not a live delivery.' : 'Testbed response, not a live delivery.'}</p></> : <div className="tracking-state"><Icon name={result?.availability === 'not_implemented' ? 'info' : 'clock'}/><div><strong>{result?.availability === 'not_implemented' ? 'Integration not implemented' : result?.availability === 'not_configured' ? 'Credentials not configured' : 'Tracking unavailable'}</strong><p>{result?.message || 'Tracking information could not be loaded.'}</p></div></div>}
